...
3. go to users.js file, we now on route /users: `router.get("/")` and we can just say `request.query.<whatever we passed in here>`so we called it name, `req.query.name`